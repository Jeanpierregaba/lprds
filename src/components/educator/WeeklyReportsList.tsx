...
export const WeeklyReportsList = ({ 
  status, 
  onViewReport, 
  onEditReport,
  refreshTrigger = 0 
}: WeeklyReportsListProps) => {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      // Map to expected WeeklyReport type
      const mappedReports: WeeklyReport[] = filteredData.map((report: any) => ({
        ...report,
        child: report.children || undefined,
        status: report.is_validated ? 'validated' as const : 
                report.status === 'rejected' ? 'rejected' as const :
                report.status === 'pending' ? 'pending' as const : 'draft' as const
      }));
      ...
      toast({
        variant: 'destructive',
        title: 'Erreur',
        description: 'Impossible de charger les rapports hebdomadaires.'
      });
    } finally {
      setLoading(false);
    }
